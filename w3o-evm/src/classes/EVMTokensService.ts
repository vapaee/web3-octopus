// w3o-antelope/src/classes/EVMTokensService.ts
...
import { EVMTransferSummary } from "../types";

const logger = new W3oContextFactory('EVMTokensService');

/**
 * Service responsible for managing token balances and transfers on EVM networks.
 */
export class EVMTokensService extends W3oService {
    ...
    override init(octopus: W3oInstance, requirements: W3oModule[], parent: W3oContext): void {
        ...
        logger.info('EVMTokensService OK!', super.w3oId);
    ...
    public transferToken(
        ...
        const result$ = new Subject<EVMTransferSummary>();
        try {
            const action = {
            ...
            auth.session.signTransaction({ action }, context).subscribe({
                next: (result) => {
                    ...
                    const summary: EVMTransferSummary = {
                        from,
                        to,
                        amount: quantity,
                        transaction: txId,
